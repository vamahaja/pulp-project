...
**Directory layout:** `deployment/` contains `deploy.sh`, `podman-compose.yaml`, `config/nginx.conf`. All deployment and management commands below are run from the **`deployment/`** directory.

## Quick Start

From the **`deployment/`** directory, run the deployment script with a base directory for persistent data:
...
From the **`deployment/`** directory, run `podman-compose` with **`PULP_BASE_DIR` set to the same path you passed to `deploy.sh`** (e.g. `/opt/pulp-data`). If it is unset or different, the compose stack may use or create a different data path and not match your existing deployment.
